...
function convert_strings () {
  find ./InVehicleDevice \( -name '*.java' -o -name '*.kt' -o -name '*.xml' -o -name '*.txt' -o -name build.gradle \) -type f | xargs sed -i s/kogasoftware/choisoko2/g
...
function rename_dirs {
  if [ -e ./InvehicleDevice/src/androidTest/java/com/kogasoftware ]; then
    mv ./InVehicleDevice/src/androidTest/java/com/{kogasoftware,choisoko2}
  fi
  if [ -e ./InvehicleDevice/src/main/java/com/kogasoftware ]; then
    mv ./InVehicleDevice/src/main/java/com/{kogasoftware,choisoko2}
  fi
}
